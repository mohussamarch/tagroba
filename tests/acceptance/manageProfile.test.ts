...
describe('ملف المستخدم — حالة الاستخدام', () => {
  ...
  it('أسئلة البداية لأي حساب ما خلصهاش: من غير ملف، أو ملف متحفظ من قسم الحساب — وبعد الإنهاء ما تظهرش تاني', async () => {
    ...
    expect(await profiles.load()).toBeNull() // السؤال ما كتبش حاجة
    await manage.save({ ...emptyProfile(), displayName: 'محمد', payday: 28 })
    expect(manage.needsOnboarding(await manage.load())).toBe(true)
    expect(await manage.completeOnboarding({ ...(await manage.load()), payday: 25 })).toMatchObject({ ok: true })
    const done = await manage.load()
    expect(done).toMatchObject({ displayName: 'محمد', payday: 25, onboardedAt: '2026-09-13T13:40:00.000Z' })
    expect(manage.needsOnboarding(done)).toBe(false)
    // حفظ عادي بعدين من قسم الحساب ما يرجّعش الأسئلة
    await manage.save({ ...done, payday: 27 })
