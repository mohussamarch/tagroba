...
export interface Transaction {
  ...
  /**
   * المحفظة المستقبِلة في التحويل الداخلي — **الطرف الثاني**.
   *
   * `spec/02`: «عملية اقتصادية واحدة لها **طرف خصم وطرف إضافة**…
   *  وعرض الدخل والمصروف لا يعيد حساب الطرفين.»
   *
   * ⚠️ لماذا هنا لا في جدول `WalletPosting` المستقل: الحالة الوحيدة التي
   * تحتاج طرفين اليوم هي التحويل الداخلي، وجدول الحركات الكامل يفرض
   * إعادة كتابة الاستيراد والمطابقة والدفتر معًا. الحقل هنا يحقق نفس
   * المعنى بأقل تغيير، ويُستبدل بالجدول حين تظهر حالة ثانية تحتاجه
   * (تحويل بعملتين مثلًا). مسجَّل في ARCHITECTURE §17.
   *
   * الرصيد: هذه العملية **خصم** من `walletId` و**إضافة** إلى هذا الحقل،
   * وليست مصروفًا ولا دخلًا في أي منهما.
   */
